...
export function ajax_supervisorlog(params) {
  ...
  return function (req, res) {
    ...
    if (!host || !process) {
      return res
        .status(400)
        .json({ result: 'error', message: 'Host and process are required' });
    }
          ...
          if (offset === 0) {
            ...
            supclient.tailProcessStdoutLog(process, 0, 0, function (err, data) {
              if (!err) {
                ...
                supclient.tailProcessStdoutLog(
                  process,
                  offset,
                  length,
                  function (innerErr, innerData) {
                    if (!innerErr) {
                      return res.json({ result: 'success', data: innerData });
                    }

                    return res.status(500).json({ result: 'error', error: innerErr });
                  }
                );
              } else {
          ...
          if (offset === 0) {
            supclient.tailProcessStderrLog(process, 0, 0, function (err, data) {
              if (!err) {
                // If we dont know the offset to start, lets do two calls to fetch the current logsize and then offset
                offset = Math.max(data[1] - length, 0);
                supclient.tailProcessStderrLog(
                  process,
                  offset,
                  length,
                  function (innerErr, innerData) {
